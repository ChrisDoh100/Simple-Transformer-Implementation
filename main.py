...
def training(training_config):
    ...
    Language_model = Transformer(config =training_config,vocab_size=training_config['vocab_size']).to(training_config['device'])
    ...
    if training_config['validation']:
        validationsampler = tokenBatchSampler(dataset=validationset,max_tokens=1500,shuffle=True)
    ...
    for epoch in range(training_config['epochs']):
        train_loop(trainingdataloder,scheduler,Language_model,label_smoothing,KVloss)
        if training_config['validation']:
            val_loop(sp,validationdataloader,Language_model,label_smoothing,KVloss)
...
if __name__=="__main__":
    ...
    parser.add_argument("--source_ext",type=str, default='de')
    parser.add_argument("--target_ext",type=str, default='en')
