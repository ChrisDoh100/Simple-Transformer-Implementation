...
def translate(model,sentence_batch, decoding_method):
    if decoding_method=='greedy':
        print(greedy_decoding(model,sentence_batch)[0])
    else:
        print("Dafuq")
        print(beam_search(model,sentence_batch,translate_config['beam_width']))
...
if __name__=="__main__":
    ...
    Translation_model = Transformer(config=translate_config,vocab_size=translate_config['vocab_size']).to(translate_config['device'])
